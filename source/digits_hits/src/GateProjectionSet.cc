...
void GateProjectionSet::Reset(size_t energyWindowNumber, size_t headNumber, size_t projectionNumber)
{
  ...
  G4int numberOfFFDProcesses = 5;
  if (theDC->GetARFSD() != 0){
    stage = theDC->GetARFSD()->GetStage();
  }
  if (stage == 2)
    {
      ...
      m_ARFdata = (ARFProjectionDataType**) malloc(m_headNb
                                                   * numberOfFFDProcesses
                                                   * sizeof(ARFProjectionDataType*));
      if (!m_ARFdata)
        G4Exception("GateProjectionSet::Reset",
                    "Reset",
                    FatalException,
                    "Could not allocate a new projection set for ARF Projections (out of memory?)\n");

      for (headID = 0; headID < m_headNb * numberOfFFDProcesses; headID++)
        {
      ...
      G4cout << " GateProjectionSet::Reset : Estimated size for the Binary Projection Output file "
             << ARFBytesPerHead() * G4double(m_headNb) / (1024. * 1024.)
             << " Mo\n";
      G4cout << " GateProjectionSet::Reset : Estimated size for the Binary Projection Output file "
             << ARFBytesPerHead() * G4double(m_headNb) / 1024.
             << " Ko\n";
      return;
    }
      ...
      for (headID = 0; headID < m_headNb; headID++)
        {
          ...
          if (!(m_data[energyWindowID][headID]))
            G4Exception("GateProjectionSet::Reset",
                        "Reset",
                        FatalException,
                        "Could not allocate a new projection (out of memory?)");
...
void GateProjectionSet::ClearData(size_t projectionID)
{
  ...
  if (theDC->GetARFSD() != 0)
    {
      arfstage = theDC->GetARFSD()->GetStage();
    }

  if (arfstage == 2)
    {
      for (size_t headID = 0; headID < m_headNb * numberOfFFDProcesses; headID++)
        {
...
{
  dest.seekp(energyWindowID * BytesPerEnergyWindow()
             + headID * BytesPerHead()
             + m_currentProjectionID * BytesPerProjection(),
             std::ios::beg);
  if (dest.bad())
    G4Exception("GateProjectionSet::StreamOut",
                "StreamOut",
                FatalException,
...
void GateProjectionSet::FillARF(G4int headID,
                                G4double x,
                                G4double y,
                                G4double ARFvalue,
                                bool addEmToArfCount)
{
  ...
  if ((size_t) headID >= m_headNb * numberOfFFDProcesses)
    {
      G4cerr << "[GateToProjectionSet::FillFromARF]:\n"
             << "Received a hit with a wrong head ID ("
             << headID
  ...
  if (headID + 1 > int(m_numberOfARFFFDHeads))
    {
      m_numberOfARFFFDHeads = headID + 1;
    }
  ...
  if (max - m_ARFdataMax[headID] > 0.)
    {
      m_ARFdataMax[headID] = max;
    }

  if (addEmToArfCount && headID == 0)
    {
      m_numberOfEmEvents++;
    }
...
{
  dest.seekp(headID * ARFBytesPerHead() + m_currentProjectionID * ARFBytesPerProjection(),
             std::ios::beg);
  if (dest.bad())
    G4Exception("GateProjectionSet::StreamOutARFProjection",
                "StreamOutARFProjection",
                FatalException,
                "Could not write a projection onto the disk (out of disk space?)!\n");
