...
void GateCoincidenceSorter::Digitize()
{
  ...
  for (std::vector<GateCoincidenceDigi*>::const_iterator it = output_vector->begin();it != output_vector->end() ; ++it){
      /*G4cout<<"coincidencePulse vector size: "<<(*it)->size()<<G4endl;
...
void GateCoincidenceSorter::ProcessCompletedCoincidenceWindow(GateCoincidenceDigi *coincidence)
{
	...
	//G4cout<<"GateCoincidenceSorter::ProcessCompletedCoincidenceWindow"<<G4endl;
	//G4cout<<coincidence<<G4endl;
...
void GateCoincidenceSorter::DescribeMyself(size_t )
{
  ;
}
