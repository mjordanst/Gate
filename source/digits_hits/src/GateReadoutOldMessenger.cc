...
GateReadoutOldMessenger::GateReadoutOldMessenger(GateReadoutOld* itsReadoutOld)
    : GatePulseProcessorMessenger(itsReadoutOld)
{
    G4String cmdName;

    // Command for choosing the depth of application of the ReadoutOld
    cmdName = GetDirectoryName()+"setDepth";
    SetDepthCmd = new G4UIcmdWithAnInteger(cmdName,this);
    SetDepthCmd->SetGuidance("Defines the 'depth' of the ReadoutOld:");
    ...
    //       we now introduce an option to choose a PMT like ReadoutOld by using a policy like "TakeTheCentroidInEnergyForFinalPosition".
    ...
    //OK: Choosing the name of the volume where ReadoutOld is applied
    cmdName = GetDirectoryName() + "setReadoutOldVolume";
    SetVolNameCmd = new G4UIcmdWithAString(cmdName,this);
    SetVolNameCmd->SetGuidance("Choose a volume (depth) for ReadoutOld (e.g. crystal/module)");

    //OK: Force the name of the volume where ReadoutOld is applied for EnergyCentroid policy
    cmdName = GetDirectoryName() + "forceReadoutOldVolumeForEnergyCentroid";
    ForceDepthCentroidCmd = new G4UIcmdWithABool(cmdName,this);
    ForceDepthCentroidCmd->SetGuidance("Force Depth For Energy Centroid:set to true to activate /setReadoutOldVolume and /setDepth even if the policy is EnergyCentroid."
...
GateReadoutOldMessenger::~GateReadoutOldMessenger()
{
  delete SetDepthCmd;
  delete SetPolicyCmd;
...
void GateReadoutOldMessenger::SetNewValue(G4UIcommand* aCommand, G4String aString)
{
  if( aCommand==SetDepthCmd )
    { GetReadoutOld()->SetDepth(SetDepthCmd->GetNewIntValue(aString)); }
  else if ( aCommand==SetVolNameCmd )
    { GetReadoutOld()->SetVolumeName(aString); }
  else if ( aCommand==ForceDepthCentroidCmd )
     { GetReadoutOld()->ForceDepthCentroid(aString); }
  else if ( aCommand==SetPolicyCmd )
    { GetReadoutOld()->SetPolicy(aString); }
  //else if ( aCommand==SetResultingXYCmd )
   //  { GetReadoutOld()->SetResultingXY(aString); }
  //else if ( aCommand==SetResultingZCmd )
  //   { GetReadoutOld()->SetResultingZ(aString); }
  else
